...
for ndir = 1:size(list_dir,2), %loop over all folders which contain keyword
    ...
    if LocationTracking == 1
        ...
        %figure
        %title('Non-interpolated heatmap for mean JD')
        %pointsize = 5;
        %scatter(plotY,plotX,pointsize,plotZ,'filled');l = colorbar; ylabel(l,'Mean JD (nm)');colormap hot;
        %set(gca,'YDir','reverse');
        %saveas(gcf,'C:\Users\Alex\Desktop\Positional tracking attempt\Heatmap.fig')
...
%vq = griddata(plotX,plotY,plotZ,X,Y);

%imagesc(plotY,plotX,vq);

%colormap hot

%[X,Y] = meshgrid(plotX,plotY);
%Zi = griddata(plotX,plotY,plotZ,X,Y);
%imagesc(X,Y,Zi);
%colormap hot
