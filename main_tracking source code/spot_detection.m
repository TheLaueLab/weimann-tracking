...
%    SNR_raw_first_frame_all = [SNR_raw_first_frame_all SNR_raw_first_frame];
%    SNR_mean_first_frame_all = [SNR_mean_first_frame_all mean(SNR_raw_first_frame)];
...
%                'MarkerFaceColor','g',...
%                'MarkerSize',10)
%    title(['raw SNR over image stack: ',num2str(mean(y)),' +- ', num2str(std(y))]);
%    fName_hist_all = strcat(save_dir,'/','SNR.fig');
%    ylim([0 max(y)])
%    ylabel('raw SNR','fontsize',12,'fontweight','b')
%    xlabel('Video index','fontsize',12,'fontweight','b')
%    saveas(gcf,fName_hist_all)

%    y = mean_pp_distance_all;
%    figure(),plot(1:1:length(number_frames_per_cell),y,'--rs','LineWidth',2,...
%                'MarkerEdgeColor','k',...
%                'MarkerFaceColor','g',...
%                'MarkerSize',10)
%    title(['mean particle-particle distance over image stack: ',num2str(mean(y)),' +- ', num2str(std(y)), ' [pixels]']);
%    fName_hist_all = strcat(save_dir,'/','density.fig');
%    ylim([0 max(y)])
